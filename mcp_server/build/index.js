...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        if (name === "erpnext_get_list") {
            const { doctype, fields, filters, limit_page_length } = args;
            const params = { limit_page_length: limit_page_length || 20 };
            if (fields)
        ...
        if (name === "erpnext_get_doc") {
            const { doctype, name: docName } = args;
