...
class _HRDashboardScreenState extends ConsumerState<HRDashboardScreen> {
    ...
    return Scaffold(
      ...
      body: RefreshIndicator(
        onRefresh: () async {
          ...
          ref.invalidate(hrDepartmentStatsProvider);
        },
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              _buildDepartmentChart(context, ref),
              const SizedBox(height: 20),
    ...
    return summaryAsync.when(
      data: (summary) => Column(
        children: [
          Row(
            children: [
              Expanded(child: _kpiCard('Total Employees', summary.totalEmployees.toString(), Icons.people_alt_outlined, const Color(0xFF3B82F6))),
              const SizedBox(width: 12),
              Expanded(child: _kpiCard('Present Today', summary.presentToday.toString(), Icons.how_to_reg_outlined, const Color(0xFF10B981))),
            ],
          ),
          const SizedBox(height: 12),
          Row(
            children: [
              Expanded(child: _kpiCard('Open Leaves', summary.leaveApplications.toString(), Icons.event_note_outlined, const Color(0xFFF59E0B))),
              const SizedBox(width: 12),
  ...
  Widget _buildDepartmentChart(BuildContext context, WidgetRef ref) {
    final deptAsync = ref.watch(hrDepartmentStatsProvider);
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(16), boxShadow: [BoxShadow(color: Colors.black.withValues(alpha: 0.03), blurRadius: 10, offset: const Offset(0, 4))]),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('Employees by Department', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Color(0xFF1E2A38))),
          const SizedBox(height: 20),
          deptAsync.when(
            data: (data) {
                      ...
                      children: data.map((e) => Padding(
                        ...
                        child: Row(
                          children: [
                            ...
                            Expanded(child: Text(e.department, style: const TextStyle(fontSize: 12, color: Color(0xFF4B5563)), maxLines: 1, overflow: TextOverflow.ellipsis)),
                            Text(e.count.toString(), style: const TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Color(0xFF1E2A38))),
                          ],
                        ),
                      )).toList(),
                    ),
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            ...
            children: [
              _actionBtn('Employees', Icons.badge_outlined, () => context.push('/hr/employees')),
              _actionBtn('Attendance', Icons.how_to_reg_outlined, () => context.push('/hr/attendance')),
              _actionBtn('Leaves', Icons.event_note_outlined, () => context.push('/hr/leaves')),
              _actionBtn('Payroll', Icons.monetization_on_outlined, () => context.push('/hr/salary-slips')),
            ],
          ),
        ],
      ),
