...
class _SalesInvoiceListScreenState extends ConsumerState<SalesInvoiceListScreen> {
  ...
  Future<void> _fetchPage(int pageKey) async {
    try {
      ...
      final items = await ds.getList(
        'Sales Invoice',
        fields: ['name', 'customer', 'customer_name', 'grand_total', 'outstanding_amount', 'status', 'posting_date', 'due_date', 'currency', 'is_return'],
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [IconButton(icon: const Icon(Icons.add), onPressed: () => context.push('/resource/Sales Invoice/new'))],
      ...
      floatingActionButton: ref.watch(userPermissionsProvider('Sales Invoice')).maybeWhen(
        data: (p) => p.canCreate == true ? FloatingActionButton(onPressed: () => context.push('/resource/Sales Invoice/new'), backgroundColor: const Color(0xFF10B981), child: const Icon(Icons.add, color: Colors.white)) : null,
        orElse: () => null,
      ),
      body: PagedListView<int, Map<String, dynamic>>(
        ...
        builderDelegate: PagedChildBuilderDelegate<Map<String, dynamic>>(
          ...
          noItemsFoundIndicatorBuilder: (_) => const Center(child: Padding(padding: EdgeInsets.all(32), child: Column(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.receipt_long_outlined, size: 48, color: Color(0xFFD1D5DB)), SizedBox(height: 12), Text('No sales invoices found', style: TextStyle(fontSize: 16, color: Color(0xFF6B7280)))]))),
        ),
      ),
    );
  }
  ...
  Widget _buildCard(Map<String, dynamic> item) {
    ...
    return Card(
      ...
      child: InkWell(
        borderRadius: BorderRadius.circular(12),
        onTap: () => context.push('/sales/invoices/$name'),
        child: Padding(
