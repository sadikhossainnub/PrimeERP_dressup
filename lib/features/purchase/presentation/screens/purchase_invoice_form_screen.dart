...
class PurchaseInvoiceFormScreen extends ConsumerStatefulWidget {
  final String? name;
  const PurchaseInvoiceFormScreen({super.key, this.name});
...
class _PurchaseInvoiceFormScreenState extends ConsumerState<PurchaseInvoiceFormScreen> {
  ...
  Widget build(BuildContext context) {
    if (widget.name == null) {
      return Scaffold(
        appBar: AppBar(title: const Text('New Purchase Invoice'), backgroundColor: const Color(0xFF1E2A38), foregroundColor: Colors.white),
        body: const Center(child: Text('Use the generic form to create a new Purchase Invoice.')),
      );
    }
    final docAsync = ref.watch(purchaseInvoiceDetailProvider(widget.name!));
  ...
  Widget _buildView(Map<String, dynamic> doc) {
    ...
    return Scaffold(
      ...
      body: CustomScrollView(
        slivers: [
          SliverAppBar(
            backgroundColor: const Color(0xFF1E2A38), foregroundColor: Colors.white, pinned: true, expandedHeight: 150,
          ...
          if (widget.name != null)
            SliverToBoxAdapter(
              child: WorkflowActionsBar(
                ...
                currentDoc: doc,
                onWorkflowApplied: () {
                  ref.invalidate(purchaseInvoiceDetailProvider(widget.name!));
                },
              ),
            ),
          SliverPadding(
